...
def _strip_ansi_from_data(data):
    """Recursively strip ANSI escape sequences from all string values in data.

    Handles nested dicts, lists, tuples, and regular string values.
    Preserves the original data structure and types.
    """
    if isinstance(data, str):
        return _ansi_re.sub("", data)
    elif isinstance(data, dict):
        return {key: _strip_ansi_from_data(value) for key, value in data.items()}
    elif isinstance(data, list):
        return [_strip_ansi_from_data(item) for item in data]
    elif isinstance(data, tuple):
        return tuple(_strip_ansi_from_data(item) for item in data)
    return data
...
class ReportLogPlugin:
    ...
    def _write_json_data(self, data):
        cleaned = _strip_ansi_from_data(data)
        try:
            json_data = json.dumps(cleaned)
        except (TypeError, ValueError):
            data = cleanup_unserializable(data)
            json_data = json.dumps(_strip_ansi_from_data(data))
